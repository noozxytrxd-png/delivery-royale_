...
// ⚔️ ระบบอาวุธ: ทุกอาวุธตกจากฟ้า, เก็บได้ครั้งเดียว และใช้แล้วหาย
const WEAPONS = {
...
function spawnWeaponDrop(type, x, z) {
    removeWeaponDrop();
    weaponDrop = { type, x, y: 10, z };
    weaponDropMesh = createWeaponDropMesh(type);
    weaponDropMesh.position.set(x, 10, z);
    scene.add(weaponDropMesh);
}
...
function syncWeaponDrop(stateDrop) {
    ...
    if (!weaponDropMesh || !weaponDrop || weaponDrop.type !== stateDrop.type) {
        spawnWeaponDrop(stateDrop.type, stateDrop.x, stateDrop.z);
    }
    weaponDrop = { ...stateDrop };
}
...
function useInventoryItem() {
    ...
    if (isHost) {
        ...
        guestConn.send({ type: 'USE_WEAPON' });
    }
}
...
function hostSpawnWeapon() {
    if (!isHost || document.getElementById('lobbyOverlay').style.display !== 'none') return;
    if (weaponDrop || Date.now() < weaponDropCooldown) return;
    const type = weaponKeys[Math.floor(Math.random() * weaponKeys.length)];
    const x = (Math.random() * 32) - 16;
    const z = (Math.random() * 32) - 16;
    weaponDropCooldown = Date.now() + 10000;
    spawnWeaponDrop(type, x, z);
    broadcastWorldState();
}

function updateWeaponDrop() {
    if (!weaponDropMesh || !weaponDrop) return;

    if (weaponDrop.y > 0.8) {
        weaponDrop.y = Math.max(0.8, weaponDrop.y - 0.12);
        weaponDropMesh.position.y = weaponDrop.y;
        weaponDropMesh.rotation.y += 0.035;
    } else {
        weaponDropMesh.position.y = 0.8 + Math.sin(Date.now() * 0.005) * 0.15;
        weaponDropMesh.rotation.y += 0.01;
    }

    if (isHost && myId && players[myId]) {
        Object.values(players).forEach(p => {
            if (!p || p.weapon || !weaponDrop) return;
            if (Math.hypot(p.x - weaponDrop.x, p.z - weaponDrop.z) < 2.2) {
                p.weapon = weaponDrop.type;
                removeWeaponDrop();
                weaponDropCooldown = Date.now() + 5000;
                broadcastWorldState();
            }
        });
    }
...
function update() {
    ...
    if (isHost && document.getElementById('lobbyOverlay').style.display === 'none') {
        if (!weaponDrop && Date.now() > weaponDropCooldown) hostSpawnWeapon();
    }
    updateWeaponDrop();
